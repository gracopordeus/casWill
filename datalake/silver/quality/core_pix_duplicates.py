...
query = f"""
    CREATE TABLE IF NOT EXISTS silver.core_pix_id_duplicates (
    ...
    CREATE OR REPLACE TEMP VIEW temp_view_silver_core_pix_id_duplicates AS (
        SELECT
    ...
    INSERT INTO silver.core_pix_id_duplicates (
        transaction_id,
        transaction_date,
        transaction_month,
        cd_seqlan,
    ...
    SELECT * FROM temp_view_silver_core_pix_id_duplicates;
    
    COPY (SELECT * FROM silver.core_pix_id_duplicates)
    TO '{PATH_SILVER}/core_pix_id_duplicates/{FILE_NAME}.parquet' (
        FORMAT PARQUET
    );
"""
comments = """
    ...
    COMMENT ON COLUMN silver.core_pix_id_duplicates.cd_seqlan IS 'Código sequencial de transações';
    COMMENT ON COLUMN silver.core_pix_id_duplicates.transaction_type IS 'Tipo da transação';
    COMMENT ON COLUMN silver.core_pix_id_duplicates.transaction_value IS 'Valor da transação';
    COMMENT ON COLUMN silver.core_pix_id_duplicates.year IS 'Partição de Ano extraído da data de movimentação';
    COMMENT ON COLUMN silver.core_pix_id_duplicates.month IS 'Partição de Mês extraído da data de movimentação';
    COMMENT ON COLUMN silver.core_pix_id_duplicates.day IS 'Partição de Dia extraído da data de movimentação';
"""
